...
def _write_csv(path: Path, header: str, row: str) -> None:
    """Write a minimal CSV file for API contract tests."""
    path.write_text(f"{header}\n{row}\n", encoding="utf-8")


def _seed_season_contract(output_dir: Path, season: int) -> None:
    """Create the first-pass UI CSV contract for one season."""
    _write_csv(
        output_dir / f"{season}_team_per_game_stats.csv",
        "team,points_for",
        "DET,510",
    )
    _write_csv(
        output_dir / f"{season}_qb_per_game_stats.csv",
    ...
    _write_csv(
        output_dir / f"{season}_combined.csv",
        "team,points_for,points_per_offensive_snap,opp_points_for,SaCR,SaOR,SaDR,SaOvR,SRS",
        "DET,510,0.42,390,1.2,1.1,0.9,1.0,7.4",
    )
    _write_csv(output_dir / f"{season}_ratings.csv", "team,SaCR", "DET,1.2")
    _write_csv(
        output_dir / f"{season}_qb_combined.csv",
        (
    ...
    _write_csv(output_dir / f"{season}_qb_ratings.csv", "player_id,QSaCR", "qb-1,1.3")
...
def test_list_seasons_returns_complete_contracts_only(tmp_path: Path) -> None:
    ...
    _write_csv(tmp_path / "2025_combined.csv", "team,SaCR", "KC,1.0")
