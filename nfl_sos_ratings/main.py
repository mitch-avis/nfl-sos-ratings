...
def main() -> None:
    ...
    qb_join_keys = (
        [
            ...
            if key in qb_season_stats.columns
            and qb_opp_profiles is not None
            and key in qb_opp_profiles.columns
        ]
        if qb_opp_profiles is not None
        else []
    )
    if not qb_join_keys and "team" in qb_season_stats.columns:
        qb_join_keys = ["team"]
    qb_combined = (
        qb_season_stats.join(qb_opp_profiles, on=qb_join_keys, how="left")
        if qb_opp_profiles is not None
        else qb_season_stats
    )
    ...
    if qb_diff_exprs:
        qb_combined = qb_combined.with_columns(qb_diff_exprs)
    ...
    for year in calibration_years:
        try:
            historical_qb = compute_qb_season_stats(
                load_qb_stats(year),
                weekly_df=load_weekly_team_stats(year),
            )
            historical_qb_frames.append(historical_qb)
        except Exception as exc:
            print(f"  WARNING: Skipping QB calibration season {year}: {exc}")

    if historical_qb_frames:
        min_corr, sos_weight = calibrate_qb_model(
